...
export class AddExperienceComponent implements OnInit, OnDestroy {
  ...
  public onSubmit(): void {
    ...
    if (this.experience().experienceId === 0) {
      this.experienceService.createExperience(this.prepareExperienceData()).subscribe({
        next: () => {
      ...
      this.experienceService.updateExperience(this.prepareExperienceData()).subscribe({
        next: () => {
          this.loadingService.dismissLoadingControl()
  ...
  private async fetchData() {
    ...
    const dataCalls = forkJoin({
      ...
      getUnitInfo: this.staticDataService.getUnits(this.user().nationalStandard.nationalStandardId),
    }).pipe(
      catchError((err) => {
  ...
  private initializeData(): void {
    ...
    this.formTitle.set(this.experience().experienceId !== 0 ? 'Update CE' : 'Add CE')
  }
  ...
  private initializeExperienceData(): void {
    ...
    this.parentAmount.set(
      this.experience().amounts.find((p) => p.unitId === this.parentUnit().unitId) ||
        new ExperienceAmount(),
    )

    this.childAmount.set(
      this.experience().amounts.find((p) => p.unitId === this.childUnit().unitId) ||
        new ExperienceAmount(),
    )
  }
  ...
  private initializeFormControls(): void {
    ...
    for (const catList of this.categoryLists()) {
      ...
      if (this.experience().experienceId && chosenCategory) {
        this.categories.push(new FormControl(chosenCategory.categoryId, [Validators.required]))
